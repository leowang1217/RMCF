...
from trainer.training_args import ParallelMode, TrainingArguments
...
class Trainer:
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if isinstance(train_dataset,torch.utils.data.dataset.IterableDataset):
            # self.train_dataset=train_dataset[self.args.process_index]
            # if self.args.world_size > 1:
                
                # train_dataset = train_dataset[self.args.process_index]
    ...
    def train(
        ...
        **kwargs,
        ...
        other_loss={}
        # other_loss = [torch.tensor(0.0).to(args.device)]*10
        # elbo_kld = torch.tensor(0.0).to(args.device)
        # elbo_rec = torch.tensor(0.0).to(args.device)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        ...
                        for k,v in other_loss_step.items():
                            if k not in other_loss:
                                other_loss[k]=torch.tensor(0.0).to(args.device)
                            other_loss[k]+=v
                        # elbo_kld += elbo_kld_step
                        # elbo_rec += elbo_rec_step
                else:
                    tr_loss_step, other_loss_step = self.training_step(model, inputs)
                    tr_loss += tr_loss_step
                    for k,v in other_loss_step.items():
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # print("before",tr_loss,other_loss)
                    self._maybe_log_save_evaluate(tr_loss,other_loss, model, trial, epoch, ignore_keys_for_eval)
                    # print("after",tr_loss,other_loss)
    ...
    def _maybe_log_save_evaluate(self, tr_loss,other_loss, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log:
            logs: Dict[str, float] = {}
            tr_loss_scalar = tr_loss.item()
            other_loss_scalar ={k:v.item() for k,v in other_loss.items()}
            # elbo_kld_scalar = elbo_kld.item()
            # elbo_rec_scalar = elbo_rec.item()
            ...
            for k,v in other_loss_scalar.items():
                logs[k] =round(v / (self.state.global_step - self._globalstep_last_logged), 4)
            # logs["elbo_kld"] = round(elbo_kld_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            # logs["elbo_rec"] = round(elbo_rec_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            if "kld" in logs and "rec" in logs:
                logs["elbo"] = logs["kld"] + logs["rec"]
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]):
        ...
        if self.use_amp:
            ...
            loss,output = self.compute_loss(model, inputs,return_outputs=True)
        other_loss = {k:v.detach() for k,v in output[1].items()}
        # elbo_kld= output[1].detach()
        # elbo_rec = output[2].detach()

        if self.args.n_gpu > 1:
            loss = loss.mean()  # mean() to average on multi-gpu parallel training
            # elbo = elbo.mean()
            other_loss = {k:v.mean() for k,v in other_loss.items()}
            # elbo_kld =elbo_kld.mean()
            # elbo_rec =elbo_rec.mean()
            
        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            # deepspeed handles loss scaling by gradient_accumulation_steps in its `backward`
            loss = loss / self.args.gradient_accumulation_steps
            other_loss = {k:v/self.args.gradient_accumulation_steps for k,v in other_loss.items()}
            # elbo_kld = elbo_kld / self.args.gradient_accumulation_steps
            # elbo_rec = elbo_rec / self.args.gradient_accumulation_steps
