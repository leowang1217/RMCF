...
class GeneralCRF(torch.nn.Module):
    def __init__(
        ...
        learning: str = Training.PIECEWISE,
        ...
        if support_ternary:
            assert learning == Training.PIECEWISE and \
                 inference == Inference.BELIEF_PROPAGATION, \
            f"only learning={Training.PIECEWISE} and inference={Inference.BELIEF_PROPAGATION} supports ternary potentials!"
    ...
    def forward(
        ...
        *,
        ...
        if masks is None:
            masks = torch.ones([batch_size,num_nodes],device=unaries.device,requires_grad=False).bool()
        if binary_masks is None:
            edge_batch_size, _, num_edges = binary_edges.shape
            binary_masks =  torch.ones([edge_batch_size,num_edges],device=unaries.device,requires_grad=False).bool()
        if ternary_edges is not None and ternary_masks is None:
            ter_edge_batch_size,_, num_ter_edges = ternary_edges.shape
            ternary_masks = torch.ones([ter_edge_batch_size,num_ter_edges],device=unaries.device,requires_grad=False).bool()
        ...
        if targets is not None:
            if self.learning == Training.PIECEWISE:
                # unary
                norm_unary = beam_unary_potentials.log_softmax(-1)
                gold_unary = norm_unary[:, :, 0].masked_fill(~masks, 0.)
                ...
                gold_bin_phis = norm_bin_phis[:, :, 0, 0].masked_fill(~binary_masks, 0.)
                ...
                if self.support_ternary:
                    ...
                    gold_ter_phis = norm_ter_phis[:, :, 0, 0, 0].masked_fill(~ternary_masks, 0.0)
                    pll = pll + gold_ter_phis.sum(-1)
                # nll
                nll = -(pll / masks.sum(-1)).mean()
                return nll
            elif self.learning == Training.PERCEPTRON:
